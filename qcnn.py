...
class QCNNArchitecture:
    ...
    def QCNN(self, params: jnp.array) -> None:
        ...
        #### extra gates
        self.UGate(params[9:12], wires=self.wires[1])
        self.UGate(params[9:12], wires=self.wires[2])
        self.UGate(params[12:15], wires=self.wires[3])
        self.UGate(params[12:15], wires=self.wires[4])

        # Entangling scheme
        self.EntanglingGate(params[15:18], wires1=self.wires[1], wires2=self.wires[2])
        self.EntanglingGate(params[15:18], wires1=self.wires[3], wires2=self.wires[4])
        ...
        # QPooling_1
        self.QPoolFilter_1(params[18:21], wires_to_measure=self.wires[0], wires_to_apply=self.wires[1])
        self.QPoolFilter_1(params[18:21], wires_to_measure=self.wires[2], wires_to_apply=self.wires[3])
        self.QPoolFilter_1(params[18:21], wires_to_measure=self.wires[4], wires_to_apply=self.wires[5])
        qml.Barrier(only_visual=True)

        # QConvNet_3
        self.UGate(params[21:24], wires=self.wires[1])
        self.UGate(params[21:24], wires=self.wires[3])
        self.UGate(params[21:24], wires=self.wires[5])

        # Entangling scheme
        self.EntanglingGate(params[24:27], wires1=self.wires[1], wires2=self.wires[3])
        self.EntanglingGate(params[24:27], wires1=self.wires[3], wires2=self.wires[5])
        qml.Barrier(only_visual=True)
        ...
        # QPooling_2
        self.QPoolFilter_1(params[24:27], wires_to_measure=self.wires[1], wires_to_apply=self.wires[3])
        qml.ArbitraryUnitary(params[27:42], wires=[self.wires[3], self.wires[5]])